...
const startCreating = async () => {
  ...
  while ( editionCount <= growEditionSizeTo ) {
    ...
    if(newLayerArray.indexOf('MaskNoMouth') == -1 && newLayerArray.indexOf('Mouth') == -1) {
      // console.log('Both mask and mouth are not applied at all!');
      continue;
    }

    // console.log(newLayerArray);
    if(newLayerArray.indexOf('Shirt') !== -1 && newLayerArray.indexOf(('Jacket')) !== -1) {
      console.log('Jacket and Shirt were applied once', newLayerArray);
    }
    if(newLayerArray.indexOf('Jerseys') !== -1 && newLayerArray.indexOf(('Jacket')) !== -1) {
      console.log('Jerseys and Jacket were applied once', newLayerArray);
    }
    if(newLayerArray.indexOf('Mouth') !== -1) {
      console.log('Mouth layer was applied')
    }
    if(newLayerArray.indexOf(('MaskNoMouth')) !== -1) {
      console.log('Mask layer was applied')
    }
    console.log('--------------------------------------------')
    ...
    let HatsElements = [];
    let WigsElements = [];
    HatsElements = fs.readdirSync(`${basePath}/layers/Hat`);
    WigsElements = fs.readdirSync(`${basePath}/layers/Wig`);
    let exist = false;
